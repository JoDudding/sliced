...
```{r}
xgboost_rs <- last_resamples(final_wkflow, resamples = train_folds, metric = targ_metric)
```

## Collect metrics for resamples

```{r}
collect_metrics(xgboost_rs)
```

## Fit on test

```{r}
final_fitted <- last_fit(xgboost_rs, split)
```
